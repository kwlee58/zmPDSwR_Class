...
```{r}
(p <- ggplot(poly.age, aes(x=x, y=y)) + geom_polygon(alpha=0.2))
(p1<- ggplot(poly.age, aes(x=x, y=y)) + geom_polygon(fill="cyan", alpha=0.2))
...
ggplot(sor.df.2, aes(x=reorder(Var1, Freq), y=Freq)) + geom_bar(stat="identity", fill="gray") +
  coord_flip() +
  theme(axis.text.y=element_text(size=rel(0.8))) +
